...
def checkout(request, artwork_id=None):
    ...
    artwork_ids = ",".join(str(item.id) for item in items_to_checkout)
    ...
    if not all([profile.first_name, profile.last_name, profile.street_address,
        ...
        return redirect('enter_address', artwork_ids=artwork_ids)
...
def order_summary(request, artwork_ids):
    ...
    artworks = Artwork.objects.filter(id__in=artwork_ids_list, sold=False) 
